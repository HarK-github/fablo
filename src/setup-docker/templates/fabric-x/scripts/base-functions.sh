...
DEFAULT_POLICY="AND('<%= primaryOrg.mspName %>.member')"
...
generateArtifacts() {
  ...
  docker run --rm --user "$(id -u):$(id -g)" \
    ...
    sh -c 'cryptogen generate --config=/config/crypto-config.yaml --output=/config/crypto \
      && cp /config/crypto/peerOrganizations/<%= primaryOrg.domain %>/msp/tlscacerts/tlsca.<%= primaryOrg.domain %>-cert.pem \
  ...
  docker run --rm --user "$(id -u):$(id -g)" \
    -v "$FABRIC_X_ROOT:/config" \
    "$TOOLS_IMAGE" \
    configtxgen --channelID <%= channel.name %> --profile OrgsChannel \
...
networkUp() {
  mkdir -p "$FABRIC_X_ROOT/data/orderers/party1-router" \
           ...
           "$FABRIC_X_ROOT/data/committer-<%= primaryOrgSlug %>/sidecar-ledger"
...
namespaceInit() {
  docker run --rm --network "$NETWORK" --user "$(id -u):$(id -g)" \
    ...
    -v "$FABRIC_X_ROOT/crypto/peerOrganizations/<%= primaryOrg.domain %>/peers/fxconfig.<%= primaryOrg.domain %>/tls:/tls:ro,Z" \
    -v "$FABRIC_X_ROOT/crypto/peerOrganizations/<%= primaryOrg.domain %>/users/User1@<%= primaryOrg.domain %>/msp:/msp:ro,Z" \
    -v "$FABRIC_X_ROOT/crypto/peerOrganizations/<%= primaryOrg.domain %>/msp/tlscacerts/tlsca.<%= primaryOrg.domain %>-cert.pem:/org-tls-ca.pem:ro,Z" \
